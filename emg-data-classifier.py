...
class Classifier(object):
	...
	def __init__(self, name="Classifier", color=(0,200,0)):
		# Add identifiers to the classifier in order to identify what model was used in different screenshots
		self.name = name
		self.color = color
...
class Live_Classifier(Classifier):
	...
	def classify(self, emg):
		if self.X.shape[0] == 0 or self.model == None:
			# return 0 if we do not have data or model, 
			return 0
...
if __name__ == '__main__':
	...
	# Set armband LED color to model color
